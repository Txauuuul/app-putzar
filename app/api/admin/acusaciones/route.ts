...
// GET - Fetch all accusations for admin (bypasses RLS if service role key available)
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('Error fetching all accusations:', error);
    return NextResponse.json(
      { error: 'Failed to fetch accusations', details: String(error) },
      { status: 500 }
    );
  }
}
